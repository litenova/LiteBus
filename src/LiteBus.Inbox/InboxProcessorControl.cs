...
/// </summary>
public sealed class InboxProcessorControl : IInboxProcessorControl, IProcessorBackgroundControl, IAsyncDisposable
{
    /// <summary>
        ...
        while (true)
        {
            ...
            lock (_sync)
            {
                if (_state != ProcessorState.Paused)
                {
                    if (_passActive)
                    {
                        throw new InvalidOperationException("The inbox processor loop attempted to start overlapping passes.");
                    }
