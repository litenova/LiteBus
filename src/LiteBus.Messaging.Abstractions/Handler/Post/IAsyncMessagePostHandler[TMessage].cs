...
{
    /// <summary>
    /// Provides a synchronous wrapper over the asynchronous <see cref="PostHandleAsync(TMessage, CancellationToken)"/> method, facilitating the post-handling process within a synchronous context by invoking the asynchronous method with the current ambient execution context.
    /// </summary>
    ...
    {
        return PostHandleAsync(message, AmbientExecutionContext.Current?.CancellationToken ?? throw new NoExecutionContextException());
    }
    ...
    Task PostHandleAsync(TMessage message, CancellationToken cancellationToken = default);
}