...
internal sealed class GatedCommandShortcut : ICommandShortcut<GatedCommand>
{
    ...
    public Task<Shortcut> TryAnswerAsync(GatedCommand message, CancellationToken cancellationToken = default)
    {
        ...
        return Task.FromResult(message.Decision == StageDecision.Answer
            ? Shortcut.Answer("already applied")
            : Shortcut.None);
    }
}
...
internal sealed class ResultlessShortcut : ICommandShortcut<CachedValueCommand>
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return Task.FromResult(message.Decision == StageDecision.Answer
            ? Shortcut.Answer("no result supplied")
            : Shortcut.None);
    }
}
...
internal sealed class IndirectAnsweringShortcut : ICommandShortcut<ICommand>
{
    ...
    public Task<Shortcut> TryAnswerAsync(ICommand message, CancellationToken cancellationToken = default)
    {
        ...
        return Task.FromResult(Shortcut.Answer("answered by the global cache"));
    }
}
...
/// <summary>
///     Answers every command, registered against <see cref="ICommand" /> rather than a concrete message.
/// </summary>
/// <remarks>
///     Registration cannot reject this: the untyped contract is correct for every result-less command beneath
///     <see cref="ICommand" />, and only wrong for the ones that produce a result. It is the case the registration
///     check deliberately leaves to dispatch, which is why the dispatch error has to name the shortcut.
/// </remarks>
internal sealed class GlobalUntypedShortcut : ICommandShortcut<ICommand>
{
    /// <inheritdoc />
    public Task<Shortcut> TryAnswerAsync(ICommand message, CancellationToken cancellationToken = default)
    {
        return Task.FromResult(Shortcut.Answer("answered globally"));
    }
}
