...
///     </para>
///     <para>
///         The default <see cref="IInboxStore" /> registration creates a <see cref="DbContext" /> per store call through
///         <see cref="IDbContextFactory{TContext}" /> and commits inbox rows immediately inside
///         <see cref="AddAsync(InboxEnvelope, CancellationToken)" />. To
    ...
    IAsyncDisposable
{
    ...
    /// </summary>
    private readonly IEfCoreInboxDbContextFactory? _operationContextFactory;
    ...
    /// <summary>
    ///     Serializes in-memory inbox leasing across all store instances in the current process.
    /// </summary>
    ...
    /// </summary>
    private readonly EntityFrameworkCoreInboxStoreOptions _options;
    ...
    /// <summary>
    ///     Initializes a new instance of the <see cref="EfCoreInboxStore" /> class using an EF Core context factory.
    /// </summary>
    /// <param name="operationContextFactory">The factory that creates one database context per store operation.</param>
    /// <param name="options">The store options.</param>
    internal EfCoreInboxStore(
        IEfCoreInboxDbContextFactory operationContextFactory,
    ...
    public Task<int> DeleteCompletedOlderThanAsync(DateTimeOffset olderThan, CancellationToken cancellationToken = default)
    {
        return ExecuteAsync(async (context, token) =>
        {
            ...
            var query = dbContext.Set<InboxMessageEntity>()
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return ExecuteAsync(async (context, token) =>
        {
            ...
            var renewalQuery = dbContext.Set<InboxMessageEntity>()
                .Where(message =>
    ...
    public Task<PersistResult> PersistAsync(IReadOnlyList<InboxEnvelope> envelopes, CancellationToken cancellationToken = default)
    {
        ...
        return ExecuteAsync(async (context, token) =>
        {
            ...
            {
                persistedMessageIds.UnionWith(await ApplyCompletedAsync(context, completed, token).ConfigureAwait(false));
            }
    ...
    public async Task<InboxAppendResult> AddAsync(InboxEnvelope envelope, CancellationToken cancellationToken = default)
    {
        ...
        return await ExecuteAsync(async (context, token) =>
        {
            ...
            {
                var stored = await FindExistingEntityAsync(
                        context,
                        envelope.Id,
                        envelope.TenantId,
                        envelope.IdempotencyKey,
    ...
    {
        ArgumentNullException.ThrowIfNull(context);
        return new EfCoreInboxStore(context, _options, false);
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            throw new InvalidOperationException("The inbox store is not configured with a context factory.");
        }
    ...
    private static async ValueTask DisposeContextAsync(IInboxDbContext context)
    {
    ...
    /// <param name="cancellationToken">A token that cancels the operation.</param>
    /// <returns>The leased command envelopes.</returns>
    private static async Task<IReadOnlyList<InboxEnvelope>> LeasePendingInMemoryAsync(
        IInboxDbContext context,
        InboxLeaseRequest request,
        CancellationToken cancellationToken)
    {
        ...
        var candidates = await context.InboxMessages
            .Where(message =>
                ...
                ((message.Status == pendingStatus || message.Status == failedStatus) && (message.VisibleAfter == null || message.VisibleAfter <= now) ||
                 message.Status == processingStatus && message.LeaseExpiresAt != null && message.LeaseExpiresAt <= now ||
                 message.Status == processingStatus && message.LeaseExpiresAt == null && message.CreatedAt < staleCutoff))
            .OrderBy(message => message.CreatedAt)
    ...
    private static InboxMessageEntity? FindLocalEntity(
        IInboxDbContext context,
        Guid messageId,
        string? tenantId,
        string? idempotencyKey)
    {
        ...
        return context.InboxMessages.Local.FirstOrDefault(message =>
    ...
    private static void RememberBatchResult(
        Dictionary<Guid, InboxEnvelope> seenIds,
        Dictionary<string, InboxEnvelope> seenIdempotencyScopes,
    ...
    private static Task<int> SaveChangesAsync(IInboxDbContext context, CancellationToken cancellationToken)
    {
        if (context is not DbContext dbContext)
        {
    ...
    /// <returns>A task that completes when the entity has been reloaded.</returns>
        ...
        DateTimeOffset? leasedAt = null)
    {
        ...
        if (envelope.Status == InboxStatus.Processing && leasedAt is not null)
        {
    ...
    private static InboxMessagePage BuildPage(List<InboxEnvelope> envelopes, int pageSize)
    {
