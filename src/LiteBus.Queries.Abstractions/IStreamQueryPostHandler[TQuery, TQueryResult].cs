...
/// <summary>
///     Represents a post-handler that executes after a specific stream query of type <typeparamref name="TQuery" /> is
///     processed,
...
///     </para>
///     <para>
///         <strong>Warning:</strong> The result stream will have already been consumed by the time
///         this handler executes. It should not be re-enumerated unless its underlying implementation supports it (e.g.,
///         if it's backed by an in-memory list).
///         To pass data from the stream handler to the post-handler, use the <see cref="IExecutionContext.Items" />
///         collection.
///     </para>
/// </remarks>
public interface IStreamQueryPostHandler<in TQuery, in TQueryResult> :
                                                                       IMessagePostHandler<TQuery, IAsyncEnumerable<TQueryResult>>
