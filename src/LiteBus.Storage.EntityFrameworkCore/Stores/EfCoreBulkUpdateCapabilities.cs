...
{
    /// <summary>
    ///     Gets a value indicating whether <see cref="EntityFrameworkQueryableExtensions.ExecuteUpdateAsync{TSource}" /> is supported.
    /// </summary>
    ...
    internal static bool SupportsExecuteUpdate(DbContext dbContext)
    {
        ...
        return provider?.Contains("InMemory", StringComparison.Ordinal) != true
            && provider?.Contains("Sqlite", StringComparison.Ordinal) != true;
    }
}
