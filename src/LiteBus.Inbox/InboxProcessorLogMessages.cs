...
internal static class InboxProcessorLogMessages
{
    ...
    private static readonly Action<ILogger, Guid, Exception?> LeaseLossPersistSkippedMessage =
        LoggerMessage.Define<Guid>(
            LogLevel.Warning,
            new EventId(1003, nameof(LeaseLossPersistSkipped)),
            "Inbox lease-loss persistence was skipped for message {MessageId} because the active lease was lost.");
    ...
    private static readonly Action<ILogger, Guid, Exception> AfterDispatchCompletedMessage =
        LoggerMessage.Define<Guid>(
            LogLevel.Warning,
            new EventId(1004, nameof(AfterDispatchCompleted)),
            "The inbox AfterDispatch hook failed for message {MessageId}; completing dispatch despite the hook failure.");
    ...
    private static readonly Action<ILogger, Guid, Exception> AfterDispatchDeadLetteredMessage =
        LoggerMessage.Define<Guid>(
            LogLevel.Warning,
            new EventId(1005, nameof(AfterDispatchDeadLettered)),
            "The inbox AfterDispatch hook failed for message {MessageId}; moving the message to dead letter.");
    ...
    private static readonly Action<ILogger, Guid, Exception?> TerminalPersistSkippedMessage =
        LoggerMessage.Define<Guid>(
            LogLevel.Warning,
            new EventId(1006, nameof(TerminalPersistSkipped)),
            "Inbox terminal persistence was skipped for message {MessageId} because the active lease was lost.");

    /// <summary>
    ///     The delegate that writes the inbox processor pass completion event.
    /// </summary>
    private static readonly Action<ILogger, double, int, int, int, int, Exception?> PassCompletedMessage =
        LoggerMessage.Define<double, int, int, int, int>(
            ...
            new EventId(1007, nameof(PassCompleted)),
    ...
    /// <summary>
    ///     Logs an inbox dispatch failure.
    /// </summary>
    /// <param name="logger">The logger receiving the event.</param>
    ...
    /// <summary>
    ///     Logs a completed inbox processor pass.
    /// </summary>
    /// <param name="logger">The logger receiving the event.</param>
    ...
    /// <param name="succeededCount">The number of completed messages.</param>
