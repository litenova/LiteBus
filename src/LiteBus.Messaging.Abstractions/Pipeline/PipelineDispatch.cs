...
public sealed class PipelineDispatch
{
    ...
    /// </summary>
    private readonly Func<IMessagePreStageHandler, object, CancellationToken, Task<PipelineDecision>>? _preHandler;
    ...
    private PipelineDispatch(
        Type contractType,
        Func<IMessagePreStageHandler, object, CancellationToken, Task<PipelineDecision>>? preHandler,
    ...
    /// <returns>
    ///     <see cref="PreStage.Guard" /> for a guard contract, <see cref="PreStage.Validator" /> for a
    ///     validator contract, <see cref="PreStage.Shortcut" /> for a shortcut contract, and
    ///     <see cref="PreStage.PreHandler" /> for anything else.
    /// </returns>
    /// <remarks>
    ...
    /// </remarks>
    public static PreStage StageFor(Type contractType)
    {
        ArgumentNullException.ThrowIfNull(contractType);

        return PipelineContracts.Find(contractType)?.Stage ?? PreStage.PreHandler;
    }
    ...
    public static PipelineDispatch? For(Type contractType)
    {
        ...
        var contract = PipelineContracts.Find(contractType);
        ...
        return contract.Family switch
        {
            PipelineFamily.PreStage => new PipelineDispatch(
                contractType,
                BindPreHandler(contract.InvokerMethodName, arguments),
                null,
                null),
            PipelineFamily.PostHandler => new PipelineDispatch(
                contractType,
                null,
                BindPostHandler(contract.InvokerMethodName, arguments),
                null),
            PipelineFamily.CompletionHandler => new PipelineDispatch(
                contractType,
                null,
                null,
                BindCompletionHandler(contract.InvokerMethodName, arguments)),
            PipelineFamily.RefusalMapper => new PipelineDispatch(
                contractType,
                null,
                null,
                null,
                BindRefusalMapper(contract.InvokerMethodName, arguments)),
            _ => null
        };
    }
    ...
    internal Task<PipelineDecision> InvokePreHandlerAsync(
    ...
    [RequiresUnreferencedCode("Pipeline dispatch closes handler contracts over registered message types.")]
    private static Func<IMessagePreStageHandler, object, CancellationToken, Task<PipelineDecision>> BindPreHandler(
        string methodName,
        Type[] arguments)
    {
        return Bind<Func<IMessagePreStageHandler, object, CancellationToken, Task<PipelineDecision>>>(methodName, arguments);
    }
    ...
    /// <returns>The bound delegate.</returns>
    [RequiresUnreferencedCode("Pipeline dispatch closes handler contracts over registered message types.")]
    private static Func<IMessagePostHandler, object, object?, CancellationToken, Task> BindPostHandler(
        string methodName,
        Type[] arguments)
    {
        return Bind<Func<IMessagePostHandler, object, object?, CancellationToken, Task>>(methodName, arguments);
    }
    ...
    /// <param name="message">The message being mediated.</param>
    /// <param name="cancellationToken">The cancellation token supplied to the mediation operation.</param>
    /// <returns>Always <see cref="PipelineDecision.Continue" />.</returns>
    internal static async Task<PipelineDecision> InvokePreHandler<TMessage>(
        IMessagePreStageHandler handler,
        object message,
        CancellationToken cancellationToken)
    ...
    /// <summary>
    ///     Runs a guard and returns its verdict as the decision the pipeline acts on.
    /// </summary>
    /// <typeparam name="TMessage">The registered message type.</typeparam>
    /// <param name="handler">The guard instance.</param>
    /// <param name="message">The message being mediated.</param>
    /// <param name="cancellationToken">The cancellation token supplied to the mediation operation.</param>
    /// <returns>The decision for a denial, or <see cref="PipelineDecision.Continue" /> when the message may proceed.</returns>
    internal static async Task<PipelineDecision> InvokeGuard<TMessage>(
        IMessagePreStageHandler handler,
        object message,
        CancellationToken cancellationToken)
        where TMessage : notnull
    {
        ...
        return verdict.ToDecision();
    }

    /// <summary>
    ///     Runs a validator and returns what it reported as the decision the stage runner collects from.
    /// </summary>
    /// <typeparam name="TMessage">The registered message type.</typeparam>
    ...
    /// <param name="cancellationToken">The cancellation token supplied to the mediation operation.</param>
    /// <returns>
    ///     A decision carrying this validator's failures, or <see cref="PipelineDecision.Continue" /> when it found nothing wrong.
    ///     The stage runner gathers these rather than acting on the first, so returning a decision here does not by itself
    ///     end the mediation.
    /// </returns>
    internal static async Task<PipelineDecision> InvokeValidator<TMessage>(
        IMessagePreStageHandler handler,
        object message,
        CancellationToken cancellationToken)
        where TMessage : notnull
    {
        ...
        return validity.ToDecision();
    }

    /// <summary>
    ///     Runs a shortcut over a message that produces no result, and returns its answer as the decision the pipeline
    ///     acts on.
    /// </summary>
    /// <typeparam name="TMessage">The registered message type.</typeparam>
    ...
    internal static async Task<PipelineDecision> InvokeShortcut<TMessage>(
        IMessagePreStageHandler handler,
        object message,
        CancellationToken cancellationToken)
        where TMessage : notnull
    {
        ...
        return shortcut.ToDecision(handler.GetType());
    }

    /// <summary>
    ///     Runs a shortcut over a message that produces a result, and returns its answer as the decision the pipeline
    ///     acts on.
    /// </summary>
    /// <typeparam name="TMessage">The registered message type.</typeparam>
    ...
    /// <param name="cancellationToken">The cancellation token supplied to the mediation operation.</param>
    /// <returns>The decision for an answer, or <see cref="PipelineDecision.Continue" /> when the mediation proceeds.</returns>
    internal static async Task<PipelineDecision> InvokeTypedShortcut<TMessage, TMessageResult>(
        IMessagePreStageHandler handler,
        object message,
        CancellationToken cancellationToken)
        where TMessage : notnull
    ...
    /// <param name="cancellationToken">The cancellation token supplied to the mediation operation.</param>
    /// <returns>A task representing the asynchronous post-handling operation.</returns>
    internal static Task InvokePostHandler<TMessage, TMessageResult>(
        IMessagePostHandler handler,
        object message,
        object? messageResult,
        CancellationToken cancellationToken)
    ...
    /// <param name="cancellationToken">The cancellation token passed to the completion stage.</param>
    /// <returns>A task representing the asynchronous completion-handling operation.</returns>
    internal static Task InvokeCompletionHandler<TMessage>(
        IMessageCompletionHandler handler,
        MessageCompletionContext context,
        CancellationToken cancellationToken)
        where TMessage : notnull
    ...
    /// <param name="cancellationToken">The cancellation token passed to the completion stage.</param>
    /// <returns>A task representing the asynchronous completion-handling operation.</returns>
    internal static Task InvokeTypedCompletionHandler<TMessage, TMessageResult>(
        IMessageCompletionHandler handler,
        MessageCompletionContext context,
        CancellationToken cancellationToken)
        where TMessage : notnull
    ...
    [RequiresUnreferencedCode("Pipeline dispatch closes handler contracts over registered message types.")]
    private static Func<IMessageRefusalMapper, object, Refusal, object?> BindRefusalMapper(
        string methodName,
        Type[] arguments)
    {
        return Bind<Func<IMessageRefusalMapper, object, Refusal, object?>>(methodName, arguments);
    }
    ...
    internal static object? InvokeRefusalMapperCore<TMessage, TMessageResult>(
