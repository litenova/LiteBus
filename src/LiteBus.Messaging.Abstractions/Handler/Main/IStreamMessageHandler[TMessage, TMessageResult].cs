...
/// <summary>
/// Represents an asynchronous message handler that yields results in the form of an asynchronous stream of <see cref="IAsyncEnumerable{TMessageResult}" />.
/// </summary>
...
public interface IStreamMessageHandler<in TMessage, out TMessageResult> : IMessageHandler<TMessage, IAsyncEnumerable<TMessageResult>>
{
    ...
    /// <summary>
    /// Processes the given message asynchronously, yielding a stream of results. Each item in the stream represents an outcome or a part of the overall result from handling the message.
    /// </summary>
    /// <param name="message">The input message that the handler processes.</param>
    /// <param name="cancellationToken">A token that can be used by the caller to request the cancellation of the asynchronous operation, allowing for responsive applications and optimized resource utilization.</param>
    /// <returns>An asynchronous enumerable stream of results. This stream can be consumed using async enumerators, enabling the processing of each item as it becomes available.</returns>
    /// <remarks>
    /// Implementers should provide the logic for processing the message and producing the streamed results within this method. The asynchronous nature of the returned stream ensures that results can be processed and consumed as they become available, without the need to wait for the entire operation to complete.
    /// </remarks>
