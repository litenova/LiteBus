...
internal static class AzureServiceBusMessageMapper
{
    ...
    /// <param name="completeAsync">The delegate that completes the broker message.</param>
    /// <param name="abandonAsync">The delegate that abandons the broker message for redelivery.</param>
    /// <param name="deadLetterAsync">The delegate that dead-letters the broker message.</param>
    ...
    internal static TransportMessage ToTransportMessage(
        ...
        new()
        {
            ...
            AckAsync = completeAsync,
            NackAsync = (requeue, token) => requeue ? abandonAsync(token) : deadLetterAsync(token)
        };
