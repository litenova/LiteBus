...
/// </summary>
public static class KafkaTransportTestInfrastructure
{
    /// <summary>
    ...
    /// <summary>
    ///     Warms up the Kafka broker so the first test consumer does not block on metadata discovery.
    /// </summary>
    /// <param name="bootstrapServers">The bootstrap servers list.</param>
    /// <returns>A task that completes when the broker responds to metadata requests.</returns>
    public static async Task WarmupBrokerAsync(string bootstrapServers)
    {
